...
class _PositionedListState extends State<PositionedList> {
  ...
  void didUpdateWidget(PositionedList oldWidget) {
    ...
    // 检查 itemCount 是否增加
    if (widget.itemCount > oldWidget.itemCount && scrollController.hasClients) {
      // 如果已在底部，调整滚动位置
      bool isAtBottom = scrollController.position.pixels >= (widget.reverse ? scrollController.position.minScrollExtent : scrollController.position.maxScrollExtent);

      if(isAtBottom){
        if(widget.reverse){
          // 记录旧的最大滚动范围
          final oldMaxScrollExtent = oldWidget.controller?.position.maxScrollExtent ?? 0.0;
          // scrollController.animateTo(-71, duration: Duration(milliseconds: 300), curve: Curves.easeInOut);

          // WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
          //   scrollController.jumpTo(scrollController.position.maxScrollExtent - oldMaxScrollExtent);
          // },);
        ...
        //     final scrollOffsetDelta = newMaxScrollExtent - oldMaxScrollExtent;
        //
        //     // 调整滚动位置
        //     scrollController.jumpTo(scrollController.offset + scrollOffsetDelta);
        // });
      }
    }
  ...
  Future<void> _adjustScrollPositionForNewItems(int oldItemCount, int newItemCount)  async {
    // 计算新添加的项目数量
    int newItems = newItemCount - oldItemCount;

    // 计算新添加项目的总高度
    double newItemsHeight = 0.0;

    // 遍历新添加的项目，累加高度
    for (int i = oldItemCount; i < newItemCount; i++) {
      final key = ValueKey(i);
      final context = registeredElements.value?.firstWhere(
            (element) => element.widget.key == key,
      );
      if (context != null) {
        final renderBox = context.renderObject as RenderBox;
        if (renderBox.hasSize) {
          newItemsHeight += renderBox.size.height;
        } else {
          // 如果还没有布局完成，等待下一帧
    ...
    if (scrollController.hasClients) {
      double offsetAdjustment = newItemsHeight;
      ...
      // 更新滚动位置
      // scrollController.jumpTo(scrollController.position.pixels + offsetAdjustment);
      scrollController.animateTo(
        scrollController.position.pixels + offsetAdjustment,
